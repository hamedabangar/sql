...
select * from vendor a inner join vendor_booth_assignments b on a.vendor_id = b.vendor_id
order by a.vendor_name, b.market_date
...
select vendor_id , count(*) from vendor_booth_assignments
group by vendor_id
...
select a.customer_id,a.customer_first_name,a.customer_last_name, sum(quantity*cost_to_customer_per_qty)
...
*/
create table temp.tmp_new_vendor as select * from vendor;

insert into temp.tmp_new_vendor (vendor_id,vendor_name,vendor_type,vendor_owner_first_name,vendor_owner_last_name) values (10,'Thomass Superfood Store','Fresh Focused','Thomas','Rosenthal')
...
select customer_id,strftime('%m',market_date) as month,strftime('%Y',market_date) as year,sum(quantity*cost_to_customer_per_qty) as year from customer_purchases
